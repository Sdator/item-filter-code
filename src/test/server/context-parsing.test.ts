...
suite("Context Parsing -> getOperator", () => {
  test("correctly returns if the operator isn't followed by a value", () => {
    const r1 = parser.getOperator(">", 0);
    const [lha1, rha1] = r1;
    assert.strictEqual(lha1, parser.Operator.Invalid);
    assert.isUndefined(rha1);

    const r2 = parser.getOperator("<  ", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.Invalid);
    assert.isUndefined(rha2);

    const r3 = parser.getOperator(">= Test", 0);
    const [lha3, rha3] = r3;
    assert.strictEqual(lha3, parser.Operator.GreaterThanEqual);
    assert.strictEqual(rha3, 3);
  });
  ...
  test("correctly handles leading whitespace", () => {
    ...
    const r2 = parser.getOperator(" >= Test", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.GreaterThanEqual);
    assert.isDefined(rha2);
    assert.strictEqual(rha2, 4);

    const r3 = parser.getOperator("\t  < Test", 0);
    const [lha3, rha3] = r3;
    assert.strictEqual(lha3, parser.Operator.LessThan);
    assert.isDefined(rha3);
    assert.strictEqual(rha3, 5);

    const r4 = parser.getOperator("\t  Test", 0);
    const [lha4, rha4] = r4;
    assert.strictEqual(lha4, parser.Operator.Equal);
    assert.isDefined(rha4);
    assert.strictEqual(rha4, 3);
  });
  ...
  test("correctly handles the '=' operator", () => {
    ...
    const r2 = parser.getOperator(" = Test", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.Equal);
    assert.isDefined(rha2);
    assert.strictEqual(rha2, 3);
  });
  ...
  test("correctly handles the '>' operator", () => {
    ...
    const r2 = parser.getOperator(" > Test", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.GreaterThan);
    assert.isDefined(rha2);
    assert.strictEqual(rha2, 3);
  });
  ...
  test("correctly handles the '<' operator", () => {
    ...
    const r2 = parser.getOperator(" < Test", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.LessThan);
    assert.isDefined(rha2);
    assert.strictEqual(rha2, 3);
  });
  ...
  test("correctly handles the '<=' operator", () => {
    ...
    const r2 = parser.getOperator(" <= Test", 0);
    const [lha2, rha2] = r2;
    assert.strictEqual(lha2, parser.Operator.LessThanEqual);
    assert.isDefined(rha2);
    assert.strictEqual(rha2, 4);
  });

  test("correctly returns when given multiple operators", () => {
    const r1 = parser.getOperator(" > = Test", 0);
    const [lha1, rha1] = r1;
    assert.strictEqual(lha1, parser.Operator.Invalid);
    assert.isUndefined(rha1);
  ...
  test("correctly returns when given an empty string", () => {
    const result = parser.getOperator("", 0);
    const [op, index] = result;
    assert.strictEqual(op, parser.Operator.Equal);
    assert.isUndefined(index);
  });

  test("correctly returns when given a whitespace-only string", () => {
    const result = parser.getOperator(" \t ", 0);
    const [op, index] = result;
    assert.strictEqual(op, parser.Operator.Equal);
  ...
  test("correctly returns when given an out-of-range index", () => {
    const result = parser.getOperator("= Test", 6);
    const [op, index] = result;
    assert.strictEqual(op, parser.Operator.Equal);
    assert.isUndefined(index);
  });

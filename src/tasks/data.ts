...
import { dataOutputRoot, dataSourceRoot } from "../common";

const filterDataPath = path.join(dataSourceRoot, "filter.json");
const filterData = JSON.parse(fs.readFileSync(filterDataPath, "utf8")) as object;

const sfxDataPath = path.join(dataSourceRoot, "sfx.json");
const sfxData = JSON.parse(fs.readFileSync(sfxDataPath, "utf8")) as object;

const uniqueDataPath = path.join(dataSourceRoot, "uniques.json");
const uniqueData = JSON.parse(fs.readFileSync(uniqueDataPath, "utf8")) as object;

const modDataPath = path.join(dataSourceRoot, "mods.json");
const modData = JSON.parse(fs.readFileSync(modDataPath, "utf8")) as object;

const suggestionDataPath = path.join(dataSourceRoot, "suggestions.json");
const suggestionData = JSON.parse(fs.readFileSync(suggestionDataPath, "utf8")) as object;

const itemDataPath = path.join(dataSourceRoot, "items.json");
let itemData: { [key: string]: string[] };
itemData = JSON.parse(fs.readFileSync(itemDataPath, "utf8")) as typeof itemData;
